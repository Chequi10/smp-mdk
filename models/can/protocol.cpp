...
namespace protocol
{
	...
	void packet_decoder::handle_pkt_state_expecting_length()
	{
		...
		{
			// std::cout << boost::format("            Longitud recibida ")<<this->last_received_char<< std::endl;
	...
	void packet_decoder::handle_pkt_state_expecting_terminator()
	{
		...
		{ // std::cout << boost::format("______Entrosad_______")<< "\n";
	...
	void packet_decoder::check_timeouts()
	{
		...
		dt = this->last_received_packet_t0 > t1 ? 1 + this->last_received_packet_t0 + ~t1 : t1 - this->last_received_packet_t0;
